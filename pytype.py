...
def main():
    ...
    count = 0
    words = []
    files = ["csvs/typing.csv", "csvs/typing1.csv",
             "csvs/typing2.csv", "csvs/anime.csv", "csvs/hard.csv"]
    ...
    while True:
        ...
        while count < 20:
            ...
            try:
                if pop_word.strip() == value["-I1-"].strip():
                    count += 1
                    end = time.perf_counter()
                    total_time += end - start
                    ans_word += value["-I1-"] + "\n"
                    ans_time += f"問{count:>02} -> {end - start:.2f}秒\n"
                    pop_word = next(gen_words)
                    window["-I1-"].update("")
